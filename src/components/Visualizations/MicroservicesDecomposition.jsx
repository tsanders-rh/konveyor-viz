...
const MicroservicesDecomposition = ({
  ...
  // Detect analysis tier based on available data
  const detectTier = () => {
    if (!data || !data.components) return { level: 1, name: 'Structure Only', color: 'grey' };

    // Check for full source
    const hasFullSource = data.components.some(c =>
    ...
    if (hasFullSource) {
      return {
        ...
        name: 'Full Source',
        color: 'green',
        description: 'Complete source code available for production-quality business logic extraction'
      };
    }

    // Check for code snippets
    const hasCodeSnippets = data.components.some(c =>
      c.codeContext && c.codeContext.stats && c.codeContext.stats.totalSnippets > 0
    );
    if (hasCodeSnippets) {
      return {
        level: 2,
        name: 'Code Snippets',
        color: 'blue',
        description: 'Code snippets available from Kantra analysis for enhanced business logic inference'
      };
    }
    ...
    return {
      ...
      name: 'Structure Only',
      color: 'orange',
      description: 'Limited to component names and file paths - configure GitHub URL for better results'
    };
  };
  ...
  return (
    <Card>
      ...
      <CardBody>
        ...
        {/* Analysis Tier Indicator */}
        <Alert
          variant={tier.level === 3 ? 'success' : tier.level === 2 ? 'info' : 'warning'}
          isInline
          title={`Analysis Tier ${tier.level}: ${tier.name}`}
          ...
          <div style={{ fontSize: '0.875rem' }}>
            ...
            {tier.level < 3 && (
              <>
                ...
                {tier.level === 1 && (
                  <>
                    <strong>Upgrade to Tier 2:</strong> Use a Kantra report with code analysis (already included if you loaded a real report)
                    <br />
                  </>
                )}
                <strong>Upgrade to Tier 3:</strong> Configure your GitHub repository URL in{' '}
                <a
                  href="#settings"
                  onClick={(e) => {
                    e.preventDefault();
                ...
                {' '}to fetch complete source code for production-quality business logic extraction.
              </>
            )}
          </div>
        </Alert>
        ...
        {!decomposition && !loading && alreadyDecomposed && (
          <Alert variant="success" isInline title="Well-Architected Application Detected" style={{ marginBottom: '16px' }}>
            Your application appears to already have a well-balanced component architecture and does not need microservices decomposition.
            Use the <strong>AI Insights</strong> tab for modernization recommendations instead.
          </Alert>
        )}

        {!decomposition && !loading && !alreadyDecomposed && (
          <EmptyState titleText="Ready to Modernize Your Architecture?" icon={CubesIcon} headingLevel="h3">
            <EmptyStateBody>
